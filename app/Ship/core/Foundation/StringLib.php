...
/*--------------------------------------------------------------------------*/
	...
	static function getClassNameFromString($classPath = '', $lowerCase = true)
	{
		...
		return $lowerCase ? self::lower($className) : $className;
	}

	static function splitStringByTag(string $tag, string $cotent){
		$paragraphs =  preg_split('#<'.$tag.'([^>])*>#', $cotent);
        return $paragraphs;
	}
}
