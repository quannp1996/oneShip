...
@section('content')
    <div class="row">
        <div class="col-lg-12">
            ...
            {!! Form::open(['url' => route('admin_banner_home_page'), 'method' => 'get', 'id' => 'searchForm']) !!}
            <div class="card card-accent-primary">
                ...
                <div class="card-footer">
                    ...
                    <a href="{{ route('admin_banner_home_page') }}" class="btn btn-sm btn-primary"><i
                                class="fa fa-refresh"></i> Reset</a>
                    <a href="{{ route('admin_banners_add_page') }}" class="btn btn-sm btn-primary"><i
                                ...
                                <td align="center">
                                    @if($item->status == 2)
                                        <a href="javascript:void(0)"
                                           data-route="{{route('admin_banners_disable', $item->id)}}"
                                           class="text-primary" onclick="admin.updateStatus(this,{{$item->id}},1)"
                                           title="Đang hiển thị, Click để ẩn"><i class="fa fa-eye"></i></a>
                                    @else
                                        <a href="javascript:void(0)"
                                           data-route="{{route('admin_banners_enable', $item->id)}}" class="text-danger"
                                           onclick="admin.updateStatus(this,{{$item->id}}, 2)"
                                           title="Đang ẩn, Click để hiển thị"><i class="fa fa-eye"></i></a>
                                    @endif
                                    <a href="{{ route('admin_banners_edit_page', $item->id) }}"
                                       class="btn text-primary"><i class="fa fa-pencil"></i></a>
                                    @if($item->status != -1)
                                        <a data-href="{{ route('admin_banners_delete', $item->id) }}"
