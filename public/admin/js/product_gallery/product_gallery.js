var app = new Vue({
    ...
    methods:{
        ...
        updatePosition:function(item){
            ...
            shop.ajax_popup(this.enpoint_gallery_pos,'POST',{id: curItem.data("id"), next: nextCurItem.data("id"), type: type},
                function(j){
                    if(j.error > 0) {
        ...
        loadImages:function(e){
            if(typeof this.object_id != 'undefined' && this.object_id > 0) {
                ...
                this.loading = true;
                shop.ajax_popup(this.enpoint_gallery_product != '' ? this.enpoint_gallery_product : this.type+'/load', 'POST', data,
                    function (json) {
                        app.loading = false;
                        if (json.error == 0) {
        ...
        imageDel:function(item){
        	if(confirm('Bạn muốn xóa ảnh?')) {
                this.loading = true;
                shop.ajax_popup(this.enpoint_gallery_remove_img, 'POST', {id: item.id, object_id:this.object_id,type:this.type},
                    function (json) {
                        app.loading = false;
                        let msg = json.msg;
                        if (json.error == 0) {
